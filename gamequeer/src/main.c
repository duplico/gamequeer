 ...
 *
 * The framebuffer is 128x128 pixels: OLED_HORIZONTAL_MAX = OLED_VERTICAL_MAX = 128,
 * matching the badge's real LCD_X_SIZE/LCD_Y_SIZE (see grlib_gfx.h's doc
 * comment on these macros -- gamequeer#297). The OLED_*_MAX constants are the
 * width/height (exclusive upper bound on the coordinate index); valid
 * coordinates run 0..MAX-1, as enforced by the bounds check in
 * grlib_gfx_driver.c.
